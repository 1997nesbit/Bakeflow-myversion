...
import type { DailyBatchItem, TimerState, BulkBatch, FulfillmentChoice } from '@/types/production'
...
export function BakerActive() {
  ...
  const availableBatches = dailyBatches.filter(b => b.quantityRemaining > 0)
  ...
  const handleAcceptOrder = async (orderId: string, method: 'from_batch' | 'bake_fresh', batchItem?: DailyBatchItem) => {
    const prev = orders
    setOrders(p => p.map(o => o.id === orderId ? { ...o, assignedTo: 'me' } : o))

    if (method === 'from_batch' && batchItem) {
      setDailyBatches(p =>
        p.map(b => b.id === batchItem.id ? { ...b, quantityRemaining: Math.max(0, b.quantityRemaining - 1) } : b)
      )
      setFulfillments(p => ({ ...p, [orderId]: { orderId, method: 'from_batch', batchItemId: batchItem.id, batchItemName: batchItem.productName } }))
      toast(`Order accepted — from batch: ${batchItem.productName}`)
    } else {
  ...
  const handleAcceptAll = () => {
    ...
    setOrders(prev => prev.map(o => o.status === 'baker' && !o.assignedTo ? { ...o, assignedTo: 'me' } : o))
      ...
      <main className="ml-64">
        ...
        <div className="p-6 space-y-6">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-5">
                ...
                <div className="space-y-2">
                  {incomingOrders.map(order => (
                    <IncomingOrderCard
                      key={order.id}
                      order={order}
                      hasBatchAvailable={availableBatches.length > 0}
            ...
            <TabsContent value="qa" className="mt-0 space-y-2">
              {qaOrders.length === 0 ? (
                <div className="rounded-2xl border-2 border-dashed border-border p-14 text-center">
