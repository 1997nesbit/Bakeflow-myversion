...
export default function CustomersPage() {
  ...
  const avgOrderValue = customers.reduce((s, c) => s + (c.totalOrders > 0 ? c.totalSpent / c.totalOrders : 0), 0) / customers.length

  const toggleGold = (id: string) => {
    setCustomers(customers.map(c => c.id === id ? { ...c, isGold: !c.isGold } : c))
  }
