              ...
              <div key={alert.order.id} className="flex items-center justify-between gap-4 rounded-xl border-2 border-red-300 bg-red-50 p-3 animate-pulse">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-red-700">Est. {alert.order.estimatedMinutes}min | {alert.minutesOver}min overdue</p>
                  </div>
                </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-5 w-full grid grid-cols-3">
            <TabsTrigger value="actions" className="flex items-center gap-2">
              ...
              {(paidOrders.length + readyDeliveryOrders.length + readyPickupOrders.length) > 0 && (
                <span className="ml-1 flex h-5 w-5 items-center justify-center rounded-full bg-secondary text-xs text-secondary-foreground font-bold">
                  {paidOrders.length + readyDeliveryOrders.length + readyPickupOrders.length}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="tracking" className="flex items-center gap-2">
              <Clock className="h-4 w-4" /> Tracking
              {(inKitchenOrders.length + dispatchedOrders.length) > 0 && (
                <span className="ml-1 flex h-5 w-5 items-center justify-center rounded-full bg-primary/20 text-xs text-primary font-bold">
                  {inKitchenOrders.length + dispatchedOrders.length}
                </span>
              )}
            </TabsTrigger>
          ...
          <TabsContent value="actions">
            <div className="grid gap-6 lg:grid-cols-3">
              ...
              <section className="rounded-2xl border-2 border-emerald-200 bg-emerald-50/50 p-4">
                ...
                {paidOrders.length === 0 ? (
                  <p className="py-8 text-center text-sm text-muted-foreground">No orders waiting</p>
                ) : (
              ...
              <section className="rounded-2xl border-2 border-blue-200 bg-blue-50/50 p-4">
                ...
                {readyDeliveryOrders.length === 0 ? (
                  <p className="py-8 text-center text-sm text-muted-foreground">No delivery orders ready</p>
                ) : (
                  <div className="space-y-3">
              ...
              <section className="rounded-2xl border-2 border-green-200 bg-green-50/50 p-4">
                ...
                {readyPickupOrders.length === 0 ? (
                  <p className="py-8 text-center text-sm text-muted-foreground">No pickup orders ready</p>
                ) : (
                  <div className="space-y-3">
                ...
                <Card key={order.id} className="cursor-pointer border-0 shadow-sm bg-card transition-shadow hover:shadow-md" onClick={() => setSelectedOrder(order)}>
                  <CardContent className="p-4">
                    ...
                    <div className="mt-2 space-y-1">
                      <p className="text-sm text-foreground truncate">{order.items.map(i => i.name).join(', ')}</p>
                    ...
                    <div className="mt-3 flex items-center justify-between border-t border-border pt-2">
                      ...
                      <Button size="sm" variant="outline" className="h-7 w-7 p-0 border-primary text-primary hover:bg-primary hover:text-primary-foreground bg-transparent" onClick={(e) => handleOpenMessage(order, e)}>
