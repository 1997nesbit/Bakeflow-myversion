...
const DEMO_USERS = [
  { username: 'manager', password: 'manager123', name: 'Manager Admin', role: 'manager' },
  { username: 'clerk', password: 'clerk123', name: 'Store Clerk Mary', role: 'clerk' },
  { username: 'james', password: 'inv123', name: 'James Kimani', role: 'clerk' },
]
...
export default function InventoryLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (user) {
        localStorage.setItem('bbr_inventory_user', JSON.stringify({ name: user.name, username: user.username, role: user.role, loggedInAt: new Date().toISOString() }))
        router.push('/inventory')
      } else if (username && password) {
        localStorage.setItem('bbr_inventory_user', JSON.stringify({ name: username, username, role: 'clerk', loggedInAt: new Date().toISOString() }))
        router.push('/inventory')
      } else {
        ...
        <CardContent className="p-8">
          ...
          <div className="mt-6 rounded-lg bg-muted/50 border border-border p-4">
            ...
            <div className="space-y-1 text-xs text-muted-foreground">
              <p><span className="font-mono text-foreground">manager</span> / <span className="font-mono text-foreground">manager123</span> - Manager (full access)</p>
              <p><span className="font-mono text-foreground">clerk</span> / <span className="font-mono text-foreground">clerk123</span> - Store Clerk Mary</p>
              <p><span className="font-mono text-foreground">james</span> / <span className="font-mono text-foreground">inv123</span> - James Kimani</p>
            </div>
          </div>
        </CardContent>
      </Card>
